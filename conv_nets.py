...
def init_uniform_kaiming(out_features: int, in_features: int) -> t.Tensor:
    return (t.rand((out_features, in_features)) * 2 - 1) / t.sqrt(t.tensor(in_features))
...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        self.weight = nn.Parameter(init_uniform_kaiming(out_features, in_features))
    ...
    def extra_repr(self) -> str:
        return (
            f"Linear(in={self.in_features}, out={self.out_features}, bias={self.bias is not None})"
        )
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    mnist_trainset, mnist_testset = get_mnist()
    mnist_trainloader = DataLoader(mnist_trainset, batch_size=args.batch_size, shuffle=True)
    mnist_valloader = DataLoader(mnist_testset, batch_size=args.batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        preds = t.cat(preds_list)
        labels = t.cat(labels_list)
        assert preds.shape == labels.shape, (preds.shape, labels.shape)
        accuracy = t.mean((preds == labels).float())
...
class Conv2d(nn.Module):
    def __init__(
        ...
        fan_in = t.sqrt(t.tensor(self.in_channels * (self.kernel_size**2)))
        # YOUR CODE HERE - define & initialize `self.weight`
        self.weight = nn.Parameter(
            (
                t.rand(self.out_channels, self.in_channels, self.kernel_size, self.kernel_size) * 2
                - 1
            )
            / fan_in
        )
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        if self.training:
            # store a running average when training in the buffer to use during inference
            mu_ = einops.reduce(x, "b c h w->c", "mean")
            self.running_mean = (1 - self.momentum) * (self.running_mean) + self.momentum * mu_
            mu_ = einops.rearrange(mu_, "c->c 1 1")

            var_ = einops.reduce((x - mu_) * (x - mu_), "b c h w->c", "sum") / (
                x.shape[0] * x.shape[2] * x.shape[3] - 1
            )
            self.running_var = (1 - self.momentum) * (self.running_var) + self.momentum * var_
            var_ = einops.rearrange(var_, "c->c 1 1")

            self.num_batches_tracked += 1
        else:
            mu_ = self.running_mean
        ...
        return (x - mu_) / t.sqrt(var_ + self.eps) * weight + bias
...
class ResidualBlock(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.left_branch = Sequential(
            ...
            ReLU(),
            Conv2d(
                in_channels=out_feats, out_channels=out_feats, stride=1, kernel_size=3, padding=1
            ),
            BatchNorm2d(out_feats),
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        output = self.left_branch(x)
        if self.right_branch is not None:
            output += self.right_branch(x)
        else:
            output += x
...
class BlockGroup(nn.Module):
    def __init__(self, n_blocks: int, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.block_with_optional = ResidualBlock(
            in_feats=in_feats, out_feats=out_feats, first_stride=first_stride
        )
        self.additional_blocks = Sequential(
            *[
...
class ResNet34(nn.Module):
    def __init__(
        ...
        self.conv1 = Sequential(
            Conv2d(in_channels=3, out_channels=out_feats0, stride=2, padding=3, kernel_size=7),
        ...
        self.layer = Sequential(
            *[
                BlockGroup(
                    n_blocks=n, in_feats=in_feats, out_feats=out_feats, first_stride=first_stride
                )
                for n, in_feats, out_feats, first_stride in zip(
                    n_blocks_per_group,
                    [out_feats0] + out_features_per_group,
        ...
        self.fc = Linear(in_features=out_features_per_group[-1], out_features=1000)
...
def predict(
    ...
    model.eval()
    logits = model(images)
    probs, _ = F.softmax(logits, dim=-1).max(dim=-1)
...
x = t.rand(4, 4).flatten()
print(x)
s = t.as_strided(x, (2, 2, 2), (2, 4, 2))

print(s)
# t = t.as_strided(x, (2, 2), (1, 2))
# %%
# Bonus: Convolution
test_input = t.tensor(
    [
